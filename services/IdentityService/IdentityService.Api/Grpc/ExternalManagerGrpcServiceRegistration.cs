        ...
        Assembly businessAssembly)
    {
        ...
        return externalManagerContracts
            .Select(contract => new ExternalManagerGrpcService(
                contract,
                FindImplementation(contract, businessImplementations)))
            .ToArray();
    }

    public static void MapExternalManagerGrpcServices(
        this IEndpointRouteBuilder endpoints,
        IEnumerable<ExternalManagerGrpcService> services)
    {
        ...
        foreach (var service in services)
        {
            mapGrpcServiceMethod
                .MakeGenericMethod(service.ImplementationType)
    ...
    private static Type FindImplementation(Type contract, IEnumerable<Type> businessImplementations)
    {
        ...
        return matches.Length switch
        {
            ...
            0 => throw new InvalidOperationException(
                $"No Business manager implements external contract {contract.FullName}."),
            _ => throw new InvalidOperationException(
                $"Multiple Business managers implement external contract {contract.FullName}: {string.Join(", ", matches.Select(type => type.FullName))}")
        };
    }

    private static bool IsBusinessServiceInterface(Type type)
    {
        if (type == typeof(IManager)
            || type == typeof(IExternalManager)
            || type == typeof(IEngine)
            || type == typeof(IDatabaseRA))
        {
            return false;
        }
        ...
        return typeof(IManager).IsAssignableFrom(type)
            ...
            || typeof(IDatabaseRA).IsAssignableFrom(type);
    }
}
